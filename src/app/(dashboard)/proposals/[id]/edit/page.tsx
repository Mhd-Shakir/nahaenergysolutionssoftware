...
export default function EditProposalPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (data && data.notes) {
          try {
            ...
            console.error('Failed to parse full proposal data');
          }
        }
      } catch (err: any) {
    ...
    loadData();
  }, [params.id, supabase, reset, toast]);
  ...
  const processSubmitData = (data: DetailedProposalData) => {
    ...
    return {
      ...
      products: data.products.map((p, i) => ({
        ...p,
        quantity: p.quantity === 'Custom' ? (raw.products[i].customQuantity || '') : p.quantity
      }))
    };
  };
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <Loader2 className="w-12 h-12 text-[#0B07D7] animate-spin" />
      </div>
    );
  }
  ...
  return (
    <div className="max-w-6xl mx-auto py-4 md:py-8 px-4">
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
        <div>
          <h1 className="text-2xl md:text-3xl font-black text-gray-900 tracking-tight">Edit Solar Proposal</h1>
          <p className="text-gray-500 mt-1 text-sm md:text-base">Modify the details to update the professional proposal document.</p>
        </div>
                    ...
                    <td className="px-8 py-3">
                      {field.name === 'Solar Module' ? (
                        <div className="flex flex-col gap-2 w-full">
                          ...
                          {watch(`products.${index}.quantity`) === 'Custom' && (
                            <input
                              {...register(`products.${index}.customQuantity` as any, { required: true })}
                              className={cn("table-input mt-1 border border-gray-200 rounded px-2 py-1")}
                              placeholder="Enter custom KW (e.g. 15 KW)"
                            />
                          )}
                        </div>
                      ) : (
